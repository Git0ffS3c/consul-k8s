...
func (c *ConnectHelper) TestConnectionFailureWithoutIntention() {
	if c.Secure {
		...
		if c.Cfg.EnableTransparentProxy {
			k8s.CheckStaticServerConnectionFailing(c.T, c.Ctx.KubectlOptions(c.T), "http://static-server")
		} else {
			k8s.CheckStaticServerConnectionFailing(c.T, c.Ctx.KubectlOptions(c.T), "http://localhost:1234")
		}
	}
}
...
func (c *ConnectHelper) TestConnectionSuccess() {
	...
	if c.Cfg.EnableTransparentProxy {
		...
		k8s.CheckStaticServerConnectionSuccessful(c.T, c.Ctx.KubectlOptions(c.T), "http://static-server")
	} else {
...
func (c *ConnectHelper) TestConnectionFailureWhenUnhealthy() {
	...
	if c.Cfg.EnableTransparentProxy {
		k8s.CheckStaticServerConnectionMultipleFailureMessages(c.T, c.Ctx.KubectlOptions(c.T), false, []string{"curl: (56) Recv failure: Connection reset by peer", "curl: (52) Empty reply from server", "curl: (7) Failed to connect to static-server port 80: Connection refused"}, "http://static-server")
	} else {
		k8s.CheckStaticServerConnectionMultipleFailureMessages(c.T, c.Ctx.KubectlOptions(c.T), false, []string{"curl: (56) Recv failure: Connection reset by peer", "curl: (52) Empty reply from server"}, "http://localhost:1234")
	}
